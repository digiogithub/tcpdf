...
if(!class_exists('TCPDF', false)) {
	 ...
	 */
	define('PDF_PRODUCER','TCPDF 3.0.013 (http://www.tcpdf.org)');
					...
					if ($unicode[$thischar['char']] == 'AL') {
						...
						if (($i > 0) AND (($i+1) < $numchars) AND
							...
							if (($thischar['char'] == 1607) AND
								...
								// Allah Word
								$chardata2[$i-2]['char'] = 65010;
								$numchars -= 2;
								for ($j=($i-1); $j < $numchars; $j++) {
									$chardata2[$j] = $chardata2[$j+2];
								}
								array_pop($chardata2);
								array_pop($chardata2);
							} else {
						...
						if($laaletter) {
							$numchars -= 1;
							for ($j=($charAL[($x-1)]['i']); $j < $numchars; $j++) {
								$chardata2[$j] = $chardata2[$j+1];
							}
							array_pop($chardata2);
						}
					}
				...
				for ($i=0; $i < ($numchars-1); $i++) {
					if (($chardata2[$i]['char'] == 1617) AND (isset($diacritics[($chardata2[$i+1]['char'])]))) {
						...
						if (isset($cw[($diacritics[($chardata2[$i+1]['char'])])])) {
							$chardata2[$i]['char'] = $diacritics[($chardata2[$i+1]['char'])];
